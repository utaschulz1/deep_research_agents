...
- **TODO, not done yet**: `multi_agent_supervisor.py`'s `supervisor` and `supervisor_tools` nodes don't currently take `config: RunnableConfig` at all, so neither the supervisor's own `supervisor_model_with_tools.ainvoke(messages)` call nor the parallel `researcher_agent_sub.ainvoke({...})` fan-out carries `session_id` — every OpenRouter call made while the supervisor is active (the majority of calls in a `research_agent_full` run) currently falls outside session grouping. This will need forwarding a manual `config` into each parallel `researcher_agent_sub.ainvoke({...}, config)` call — it's the one spot config won't auto-propagate, since `researcher_agent_sub` is invoked with a manual `.ainvoke()` call from inside a node rather than added as a declarative LangGraph subgraph node. (Contrast with `research_agent_full.py`, which adds `supervisor_agent` — the compiled supervisor graph — directly via `add_node("supervisor_subgraph", supervisor_agent)`; that *is* LangGraph's native subgraph pattern, so config propagates into it automatically with no special handling.) `researcher_agent_sub` is compiled without its own checkpointer (see the architectural note under point 5 below), so this manual forward will be the only way its nodes learn which thread they belong to. Fold this into whichever commit next redesigns `multi_agent_supervisor.py` — same pattern as everywhere else, just not applied here yet.
...
- **`tavily_search`** (`utils.py`) is now brief-aware: `tool_node` injects `research_topic`, `checklist`, and `already_visited` (the running `visited_urls` set) as `InjectedToolArg`s the LLM never sees or supplies — same hand-rolled special-casing pattern `research_agent_mcp.py` already used for `think_tool` vs MCP tools. For each unique URL in a search's results (already-visited URLs are skipped entirely, no re-extraction), it calls `extract_relevant_content()` — brief-aware, returns a `RelevantExtraction` (`relevant: bool`, `extracted_content: str` verbatim, `covers: list[str]` — which checklist items this page addresses). Returns `(formatted_text, list[extraction_record])`, not a bare string; `tool_node` unpacks this, appends the extraction records to state's `extractions` field, and folds newly-seen URLs into `visited_urls` (`Annotated[set[str], operator.or_]`) — this is the **cross-call link dedup**: revisiting the same URL across search rounds within one sub-agent run is now prevented, where before it was unmetered.
  - **Known constraint**: the dedup is cross-*round*, not cross-*call-within-a-round*. `tool_node` reads `visited_urls` once as a snapshot before dispatching the round's tool calls (now concurrent via `asyncio.gather`), so if the LLM issues two `tavily_search` calls in the same round (e.g. one query per checklist entity) and they surface an overlapping URL, both independently run extraction on it. This is an accepted trade-off, not a bug to fix reflexively: extraction is query-anchored (`search_query` is the primary relevance anchor in `extract_relevant_content_prompt`, with `research_topic` as broader context), so two differently-worded queries hitting the same URL can legitimately pull out different, complementary excerpts rather than pure duplicates. It only becomes wasted work when the two queries are similar enough to want the same excerpt from the page. Revisit (e.g. an in-round "claimed" set) if this shows up as a real cost problem in practice.
...
- **Safety net** (`config.py` settings, all env-overridable): `max_subagent_links` (10), `max_subagent_tool_iterations` (11, incremented once per `tool_node` call — deliberately a different field from the supervisor's own `max_researcher_iterations`, a different scope entirely, to avoid recreating the naming confusion that caused this redesign), `subagent_time_budget_seconds` (600), `subagent_call_timeout_seconds` (90, wraps `derive_checklist`'s and `llm_call`'s `ainvoke`, `tavily_client.search()`, and each per-URL extraction call in `asyncio.wait_for`). `llm_call` checks the link/iteration/time caps itself, *before* calling the model at all — a capped run skips straight to `finalize_research` without spending one last, immediately-discarded round-trip. These **do** override the model's own tool-call decision — a resource cap, not a quality judgment. A timed-out `llm_call` routes straight to `finalize_research` too, rather than raising or fabricating a fake `AIMessage` — the latter would get written into `researcher_messages`, which is checkpointed per-thread, and replayed to the model on a later resume as if it were its own genuine past reasoning. A timed-out/errored per-URL extraction retries once, then falls back to a raw (unextracted) excerpt rather than dropping the URL's content entirely; it's still marked visited so it isn't reprocessed forever.
- **`stop_reason`** (`ResearcherState`/`ResearcherOutputState`, one of `"model_decided" | "llm_timeout" | "link_cap" | "iteration_cap" | "time_budget"`) — `llm_call` is the *only* place this is ever set, as part of the `Command(goto=..., update={"stop_reason": ...})` it returns on every path that ends research (resource cap, timeout, or the model declining to call more tools); `finalize_research` just reads `state["stop_reason"]`, guaranteed present by construction. This node owns routing directly via `Command` — same pattern `multi_agent_supervisor.py`'s `supervisor`/`supervisor_tools` already use — rather than a separate `add_conditional_edges` function, specifically because an earlier version computed the same "why did we stop" logic independently in two places (a conditional-edge function for routing, `finalize_research` for persisting), and since that helper read `time.time()` fresh each call, the two could actually disagree near the `subagent_time_budget_seconds` boundary if real time elapsed between them — mislabeling a genuine model-decided stop as a forced cutoff. Deciding once, in one node, makes that race structurally impossible. Only `"model_decided"` means the model judged it had enough; the other four are a forced cutoff. This exists so nothing downstream (a report, a log line, a supervisor-facing summary) can misread a resource-cap-forced stop as the model being satisfied — e.g. hitting `max_subagent_links` must never read as "research complete" in the same sense as the model genuinely stopping.
...
- `LOG_LEVEL` (default `INFO`) — passed to `logging.basicConfig` once, at import time in `api/main.py`, before the app object is created. Every module just does `logger = logging.getLogger(__name__)` and inherits this via the root logger; no per-module setup needed. A `Settings` field validator upper-cases and validates the value against real `logging` level names at `Settings()` construction time, so a lowercase value (`debug`) or a typo fails fast with a clear pydantic error instead of `logging.basicConfig` crashing the app import with a more obscure `ValueError`. `research_agent_sub.py`/`utils.py` log each node's key decisions (checklist derived, tool calls dispatched, search/extraction results and failures, and `finalize_research`'s `stop_reason` — at `WARNING` when it's a forced cutoff, `INFO` when the model genuinely decided it was done), all consistently tagged `thread=<thread_id>` where available (both files use this exact tag now, even though `utils.py`'s underlying parameter is still named `session_id` — it's the same LangGraph thread_id reused as OpenRouter's session_id, just tagged consistently for log greppability). `multi_agent_supervisor.py`'s one error-path `print()` was converted to `logger.error(...)` too, but untagged — that module doesn't take `config: RunnableConfig` yet (same TODO blocking `session_id`/`coverage_gaps`/`stop_reason` forwarding), so no `thread_id` is available there. The rest of that module (and other agents) still isn't converted, since only `research_agent_sub` has had a full logging pass so far.
