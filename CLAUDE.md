...
- **Safety net** (`config.py` settings, all env-overridable): `max_subagent_links` (10), `max_subagent_tool_iterations` (11, incremented once per `tool_node` call — deliberately a different field from the supervisor's own `max_researcher_iterations`, a different scope entirely, to avoid recreating the naming confusion that caused this redesign), `subagent_time_budget_seconds` (600), `subagent_call_timeout_seconds` (90, wraps `derive_checklist`'s and `llm_call`'s `ainvoke`, `tavily_client.search()`, and each per-URL extraction call in `asyncio.wait_for`). Unlike the coverage flag above, `should_continue`'s link/iteration/time checks **do** override the model's own tool-call decision — this is a resource cap, not a quality judgment. A timed-out `llm_call` sets a `timed_out` flag on state (checked first, before any message is read) rather than raising or fabricating a fake `AIMessage` — the latter would get written into researcher_messages, which is checkpointed per-thread, and replayed to the model on a later resume as if it were its own genuine past reasoning. A timed-out/errored per-URL extraction retries once, then falls back to a raw (unextracted) excerpt rather than dropping the URL's content entirely; it's still marked visited so it isn't reprocessed forever.
- **`stop_reason`** (`ResearcherState`/`ResearcherOutputState`, one of `"model_decided" | "llm_timeout" | "link_cap" | "iteration_cap" | "time_budget"`) — `finalize_research` records *why* the run stopped, computed by a shared `_stop_reason(state)` helper that both `should_continue` (routing) and `finalize_research` (persisting to state) call independently, since a LangGraph conditional-edge function can't itself return a state update. Only `"model_decided"` means the model judged it had enough; the other four are a forced cutoff. This exists so nothing downstream (a report, a log line, a supervisor-facing summary) can misread a resource-cap-forced stop as the model being satisfied — e.g. hitting `max_subagent_links` must never read as "research complete" in the same sense as the model genuinely stopping.
