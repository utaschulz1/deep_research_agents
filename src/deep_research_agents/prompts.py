...
extract_relevant_content_prompt = """You were searching for {search_query} while researching {research_topic}. Unlike a generic summary, your job is to pull out ONLY what is relevant to your search_query and also anything else on this page clearly important to the research_topic — and to preserve it verbatim, not paraphrase it away.
...
1. Decide whether this webpage contains information relevant to the search_query above. If it does not, set "relevant" to false and leave "extracted_content" empty — do not force irrelevant content in.
2. If relevant, extract the specific facts, figures, names, dates, and statements that bear on the search_query. Preserve them VERBATIM — do not paraphrase, round numbers, or rewrite quotes in your own words. Losing a specific number or name here is worse than including one extra sentence of context.
3. If the checklist above is non-empty, note which checklist items (if any) this content addresses in "covers" — an item only belongs there if this specific webpage actually contains information about it, not just because it's topically related.
4. Do not add commentary, don't summarize what the page is "about" in general terms — extract the substance.

Today's date is {date}.
"""

derive_checklist_prompt = """You are preparing to research the following topic. Before searching, identify whether it has distinct sub-topics or entities that each need their own coverage to fully answer it.
...
Examples:
- "How is the EU machine directive implemented in Germany vs Portugal?" -> checklist: ["Germany", "Portugal"]
- "Compare OpenAI, Anthropic, and DeepMind's approaches to AI safety" -> checklist: ["OpenAI", "DeepMind", "Anthropic"]
- "What are the best espresso machines under $500?" -> checklist: [] (single unified question, no natural sub-parts)

Return a checklist of the distinct sub-topics/entities that must each be covered, or an empty list if the topic doesn't naturally decompose that way. Don't invent sub-parts the topic doesn't actually have.
